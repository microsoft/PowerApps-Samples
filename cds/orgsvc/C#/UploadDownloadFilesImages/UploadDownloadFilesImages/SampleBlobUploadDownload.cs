...
{
    public class SampleBlobUploadDownload
    {
        private CrmServiceClient _service;
        private const int _chunkSize = 4 * 1024 * 1024; //4 MB
        public SampleBlobUploadDownload(CrmServiceClient service)
        {
            _service = service;
        }

        public Guid Upload(Entity e, string filePath, string fileName, string regardingFieldName = null)
        {

            var uploadToken = InitializeFileUpload(e, fileName, regardingFieldName);
            var chunksLists = UploadFile(e, uploadToken, filePath, fileName, regardingFieldName);
            return CommitFile(e, chunksLists, uploadToken, fileName, regardingFieldName);
        }

        private string InitializeFileUpload(Entity e, string fileName, string regardingFieldName)
        {
            ...
            else
            {
                ...
                initializeUploadRequest = new InitializeFileBlocksUploadRequest
                {
                    ...
                    FileAttributeName = regardingFieldName,
                    FileName = fileName
                };
            }
        ...
        private List<string> UploadFile(Entity e, string continuationToken, string filePath, string fileName, string regardingFieldName)
        {
        ...
        private Guid CommitFile(Entity e, List<string> chunksLists, string continuationToken, string fileName, string regardingFieldName)
        {
            try
            {
        ...
        private void DownloadFile(Entity e, string downloadToken, long fileSizeDownload, string downloadLocation, string downloadFileName)
        {
