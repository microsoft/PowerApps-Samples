...
export class ConditionalOperationsSample {
   ...
   async Run() {
      try {
         ...
         accountRecord = await this.#conditionalGetUnChanged(accountRecord, "accountid");
         ...
         accountRecord = await this.#conditionalGetChanged(accountRecord, "accountid");

         // Store the updated etag value for later use
         const updatedETagValue = accountRecord["@odata.etag"];

         this.#util.appendMessage(`Original etag value:  <code> ${initialETagValue}</code> Updated etag value: <code>${updatedETagValue}</code>`);

         // Section 2: Optimistic concurrency on delete and update
         this.#util.appendMessage("<h2>2: Optimistic concurrency on delete and update</h2>")
         // This should fail because the record has been changed
         this.#util.appendMessage("Attempting to delete the account record with the <strong>original</strong> etag value: " + initialETagValue);
         await this.#tryDelete(accountRecord.accountid, initialETagValue);
         // This should fail because the record has been changed
         this.#util.appendMessage("Attempting to update the account record with the <strong>original</strong> etag value: " + initialETagValue);
         await this.#tryUpdate(accountRecord.accountid, initialETagValue);
         // This should succeed because the etag value is current.
         this.#util.appendMessage("Updating the account record with the <strong>updated</strong> etag value: " + updatedETagValue);
   ...
   async #createRetrieveAccount() {
      ...
      try {
         ...
         this.#util.showError("Failed to create and retrieve account." + e.message);
         throw e;
      }
   }
   ...
   async #conditionalGetUnChanged(accountRecord, primaryKeyName) {
      try {
         ...
         this.#util.appendMessage("Attempted to refresh the account record, but the data is not changed.");
   ...
   async #updatePhoneNumber(accountId) {
      ...
      try {
         await this.#client.SetValue(
            ...
            "telephone1",
            newPhoneNumber);
         this.#util.appendMessage(`Updated the account record's telephone number to ${newPhoneNumber}`);
      } catch (e) {
   ...
   async #conditionalGetChanged(accountRecord, primaryKeyName) {
      try {
         ...
         const record = await this.#client.Refresh(accountRecord, primaryKeyName);
         this.#util.appendMessage("The account record now has a new phone number and etag value.");
         const table = this.#util.createTable(record, false);
         this.#container.appendChild(table);
         return record;
      } catch (e) {
   ...
   async #tryDelete(accountId, etagValue) {
      try {
         ...
         this.#util.appendMessage("Deleted the account record with etag value: " + etagValue);
      } catch (e) {
         this.#util.showExpectedError("As expected, failed to delete account record." + e.message);
         // Don't throw e;
      }
   }
   ...
   async #tryUpdate(accountId, etagValue) {
      ...
      try {
         ...
         await this.#client.Update(
            ...
            newData,
            etagValue);
         this.#util.appendMessage("Updated the account record with etag value: " + etagValue);
      }
      catch (e) {
         this.#util.showExpectedError("As expected, failed to update account record." + e.message);
         // Don't throw e;
      }
   }
   ...
   async #getRecord(accountId) {
      try {
         ...
         const record = await this.#client.Retrieve(
            ...
            accountId,
            "$select=name,revenue,telephone1,description");
         this.#util.appendMessage("Retrieved the account record with updated values:");
         const table = this.#util.createTable(record, false);
         this.#container.appendChild(table);
      } catch (e) {
