...
export class QueryDataSample {
   ...
   async #retrieveMultiLevelExpands() {
      ...
      const filters = [
         `regardingobjectid_contact_task/_accountid_value eq ${this.#contosoAccountId
         }`,
      ];
   ...
   async #retrieveAnnualIncomeAggregates() {
      ...
      const entitySetName = `accounts(${this.#contosoAccountId
         })/contact_customer_accounts`;
      const query = parameters.join("&");
   ...
   async #retrieveSecondPageOfContactsWithFetchXml(fetchXmlPage1, pagingCookie) {
      ...
      if (pagingCookie) {
         ...
         // Uncomment this when we know how to properly set the paging-cookie attribute
